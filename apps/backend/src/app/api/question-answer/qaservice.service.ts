...
export class QaserviceService {
  ...
  async createQAfromFrontend(createQADto: CreateQADto, content_id: string) {
    const content = await this.contentRepository.findOne({
      where: { content_id },
      relations: ['qa'],
    });
    ...
    const qaSave = await this.qaRepository.save(qa);
    const questions = new QaQuestion();
    questions.questionTitle = createQADto.questions[0].questionTitle;
    questions.answer = createQADto.questions[0].answer;
    questions.qa = qaSave;
